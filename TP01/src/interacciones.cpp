...
void mostrarAhorcado(std::string descubierta, int vidasRestantes) {
	std::cout << descubierta << std::endl << "Te quedan " << vidasRestantes
			<< " vidas para adivinar una palabra con " << descubierta.length()
			<< " letras" << std::endl;
}
...
void mostrarQueNoContiene(char letra) {
	std::cout << "La letra " << letra
			<< " no está en la palabra, consumiste una vida" << std::endl;
}
